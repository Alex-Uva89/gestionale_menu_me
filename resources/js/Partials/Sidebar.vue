...
const emit = defineEmits(['venue', 'value-button'])
...
function selectDashboardComponent(e, component) {
   const el = e.target;

   if(el.classList.contains('isVenue')){

      if (!component) {
         console.error('component undefined');
         return;
      }
      emit('venue', component);
   } else if(el.classList.contains('isComponent')) {

      if (!component) {
         console.error('dashboard component undefined');
         return;
      }
      emit('value-button', component);
   }

}

// function selectDashboardComponent(value) {
// }
...
<aside id="sidebar-multi-level-sidebar" class="flex flex-col top-0 left-0 z-40 w-64 h-screen transition-transform -translate-x-full sm:translate-x-0 border-e-2 border-black" aria-label="Sidebar">
   <div class="header-sidebar px-3 overflow-y-auto bg-gray-50 dark:bg-gray-800">
      ...
      <ul class="max-h-96 overflow-y-scroll scrolling space-y-2 font-medium">
         <li v-for="component in components.filter(c => c.visibility)" :key="component.id">
            <!-- venues -->
            <template v-if="component.nature === 'venues'">
               <button  type="button" @click="toggleDropdown" class="flex items-center w-full p-2 text-base text-gray-900 transition duration-75 rounded-lg group hover:bg-gray-100 dark:text-white dark:hover:bg-gray-700" aria-controls="dropdown-example" data-collapse-toggle="dropdown-example">
                  ...
                  <span class="flex-1 ms-3 text-left rtl:text-right whitespace-nowrap">{{ component.name }}</span>
                  <span class="material-symbols-outlined">arrow_drop_down</span> <!--TODO: inserire arrow_drop_up -->
               </button>
               <ul id="dropdown-example" class="hidden py-2 space-y-2">
                  <!-- {{ console.log('DEBUG', component.data) }} -->
                  <li v-for="venue in component.data" :key="venue.id">
                     <a @click="selectDashboardComponent($event, venue)" href="#" class="isVenue flex items-center w-full px-2 text-gray-900 transition duration-75 rounded-lg pl-11 group dark:text-whit hover:bg-blue-100">
                        {{ venue.name }}
                     </a>
                  </li>
               </ul>
            ...
            <!-- other buttons -->
            <template v-else>
               <a @click="selectDashboardComponent($event, component)" class="cursor-pointer flex items-center p-2 text-gray-900 rounded-lg dark:text-white hover:bg-gray-100 dark:hover:bg-gray-700 group">
                  <span class="material-symbols-outlined">{{ component.icon }}</span>
                  <span class="isComponent flex-1 text-color ms-3 whitespace-nowrap">
                     {{ component.name }}
                  </span>
               </a>
            </template>

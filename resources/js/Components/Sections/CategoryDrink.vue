...
<template>
    <section class="overflow-x-hidden">
        ...
        <div v-for="category in category_venues" class="last:mb-10 last:border-b-2  border-l-2 border-r-2  border-black"
            :key="category.id">
            <div v-if="category.is_drink" class="tab category  bg-white">
                <div class="flex max-h-14 bg-white justify-evenly items-center gap-3 p-2">
                    ...
                    <Switch_button :value="category.is_active === 1 || category.is_active === true"
                        @switchChanged="value => updateIsShowStatus(category.id, value)" />
                </div>

                <div class="bg-yellow-500" :key="componentKeyli">
                    <input type="checkbox" name="accordion-1" :id="'cb' + category.id">
                    <label :for="'cb' + category.id"
                        class="tab__label uppercase text-white text-center font-bold cursor-pointer">
                        <span class="rounded-2xl text-black w-20 h-8 bg-white p-1 flex justify-center items-center">{{
                            activeDrinksCount[category.id] }} / {{ category.drinks.length }}</span> <span>
                            {{ category.name }}
                        </span>
                    ...
                    <div class="tab__content bg-white">
                        <ul class="max-w-full">
                            <li v-for="drink in category.drinks" :key="drink.id" class="border-b-2 border-black p-2">
                                <div class="w-full h-full" :class="drink.is_active ? 'opacity-100' : 'opacity-20'">
                                    <div @click="openShowDrink(drink)" class="container-drinks px-3 cursor-pointer">
                                        <div class="flex">
                                            <img :src="drink.image == null || drink.image === '' || drink.image === 'undefined' ? 'img/defaultDish.jpg' : drink.image"
                                                alt="drink image" class="sm:max-h-32 md:max-h-40 object-cover p-1">
                                        </div>
                        ...
                        <div type="button" @click="addDrink(category.id)"
                            class="p-4 flex justify-start items-center gap-2 cursor-pointer">
    ...
    <ModalAction :showModal="showAddDrinksModal" :key="keyComponent">
        <div class="modal-confirm relative">
            <ButtonCss hoverColor="#DC2626" @click="showAddDrinksModal = false"
            ...
            <form @submit.prevent="confirmAddDrinks">
                <div class="grid-show-drink">
                    <div class="h-fit  p-2 border-2 border-black flex items-center justify-between"
                        style="grid-area: nome;">
                    ...
                    <div class="min-h-30 p-2 border-2 border-black" style="grid-area: immagine;">
                        ...
                        <img :src="imagePreview == null ? defaultImgDish : imagePreview"
                            class="h-image my-2 border border-3 border-black object-cover">
                    </div>
                    <div class="h-fit flex items-center p-2 border-2 border-black" style="grid-area: allergeni;">
                        ...
                        <ul class="flex gap-2">
                            <template v-for="allergen in allergens">
                                <li v-if="allergen.is_active" class="rounded-full cursor-pointer"
                                    @click="toggleAllergen(allergen.id)" :key="allergen.id">
                                    <img :src="allergen.icon" :alt="allergen.name + ' icon'" :id="allergen.id"
                                        class="object-scale-down w-10 h-10 rounded-full border border-3 border-black">
                                </li>
                            </template>
                            <li v-if="!allergens.some(allergen => allergen.is_active)"
                                class="w-full ps-2 font-black uppercase text-red-600 underline decoration-4 underline-offset-4 text-center">
                ...
                -->
                    <div class="h-fit  p-2 border-2 border-black flex items-center justify-between"
                        style="grid-area: ingredienti;">
                        <div class="w-full flex flex-col gap-2">
                            <div class="font-black uppercase">
                                ingredienti:
                            </div>
                            <input class="w-full h-8" type="text" v-model="drink_category.description"
                                placeholder="ingredienti">
                        </div>

                    </div>
                    <div class="h-fit p-2 border-2 border-black flex items-center justify-between"
                        style="grid-area: gradi;">
                        <div class="flex flex-col gap-2 w-full">
                            <div class="font-black uppercase">
                                gradi:
                            </div>
                            <input class="w-full h-8" type="number" v-model="drink_category.degrees"
                                placeholder="gradi">
                        </div>
                    </div>
                    <div class="h-fit p-2 border-2 border-black flex items-center justify-between"
                        style="grid-area: prezzo;">
                        <div class="flex flex-col gap-2 w-full">
                            <div class="font-black uppercase">
                                prezzo:
                            </div>
                    ...
                    <div class="h-fit  p-2 border-2 border-black flex items-center justify-between"
                        style="grid-area: produzione;">
                        <div class="w-full flex flex-col gap-2">
                            <div class="font-black uppercase">
                                metodo di produzione:
                            </div>
                            <select class="w-full h-8" v-model="drink_category.production_method"
                                placeholder="Scegli un metodo">
                    ...
                    <div class="h-fit  p-2 border-2 border-black flex items-center justify-between"
                        style="grid-area: vitigno;">
                        <div class="w-full flex flex-col gap-2">
                            <div class="font-black uppercase">
                                vitigno:
                            </div>
                            <input class="w-full h-8" type="text" v-model="drink_category.grape_variety"
                                placeholder="vitigno">
                        </div>
                    </div>
                    <div class="h-fit  p-2 border-2 border-black flex items-center justify-between"
                        style="grid-area: produttore;">
                        <div class="w-full flex flex-col gap-2">
                            <div class="font-black uppercase">
                                produttore:
                            </div>
                            <input class="w-full h-8" type="text" v-model="drink_category.producer"
                                placeholder="produttore">
                        </div>
                    </div>
                    <div class="h-fit  p-2 border-2 border-black flex items-center justify-between"
                        style="grid-area: denominazione;">
                        <div class="w-full flex flex-col gap-2">
                            <div class="font-black uppercase">
                                denominazione:
                            </div>
                            <input class="w-full h-8" type="text" v-model="drink_category.denomination"
                                placeholder="denominazione">
                        </div>
                    </div>
                    <div class="h-fit  p-2 border-2 border-black flex items-center justify-between"
                        style="grid-area: annata;">
                        <div class="w-full flex flex-col gap-2">
                            <div class="font-black uppercase">
                                annata:
                            </div>
                            <input class="w-full h-8" type="number" v-model="drink_category.vintage"
                                placeholder="annata">
                        </div>
                    </div>
                    <div class="h-fit  p-2 border-2 border-black flex items-center justify-between"
                        style="grid-area: allevamento;">
                        <div class="w-full flex flex-col gap-2">
                            <div class="font-black uppercase">
                                metodo di allevamento:
                            </div>
                            <input class="w-full h-8" type="text" v-model="drink_category.breeding_method"
                                placeholder="metodo di allevamento">
                        </div>
                    </div>
                    <div class="h-fit  p-2 border-2 border-black flex items-center justify-between"
                        style="grid-area: formato;">
                        <div class="w-full flex flex-col gap-2">
                            <div class="font-black uppercase">
                                formato:
                            </div>
                    ...
                    <div class="h-fit  p-2 border-2 border-black flex items-center justify-between"
                        style="grid-area: temperatura;">
                        <div class="w-full flex flex-col gap-2">
                            <div class="font-black uppercase">
                                temperatura di servizio:
                            </div>
                            <input class="w-full h-8" type="number" v-model="drink_category.serving_temperature"
                                placeholder="temperatura di servizio">
                        </div>
                    </div>
                    <div class="h-fit  p-2 border-2 border-black flex items-center justify-between"
                        style="grid-area: olfatto;">
                        <div class="w-full flex flex-col gap-2">
                            <div class="font-black uppercase">
                                olfatto:
                            </div>
                    ...
                    <div class="h-fit  p-2 border-2 border-black flex items-center justify-between"
                        style="grid-area: certificazioni;">
                        <div class="w-full flex flex-col gap-2">
                            <div class="font-black uppercase">
                                certificazioni:
                            </div>
                            <input class="w-full h-8" type="text" v-model="drink_category.certifications"
                    ...
                    <div class="h-fit  p-2 border-2 border-black flex items-center justify-between"
                        ...
                        <div class="w-full flex flex-col gap-2">
                            <div class="font-black uppercase">
                                collegamento a drinkPuglia:
                            </div>
                            <input class="w-full h-8" type="text" v-model="drink_category.url"
                                placeholder="collegamento url">
                        </div>
                    </div>
                </div>
    ...
    <ModalAction :showModal="showEditModal">
        <div class="modal">
            ...
            <div class="flex flex-col gap-4 justify-between items-center">
                ...
                <input type="text" id="categoryName" v-model="category_venues.name"
                    :placeholder="category_venues.find(category => category.id === categoryToEdit).name">

                <label for="categoryNameEn">Inglese:</label>
                <input type="text" id="categoryNameEn" v-model="category_venues.name_en"
                    :placeholder="category_venues.find(category => category.id === categoryToEdit).name_en">

                <label for="categoryNameFr">Francese:</label>
                <input type="text" id="categoryNameFr" v-model="category_venues.name_fr"
                    :placeholder="category_venues.find(category => category.id === categoryToEdit).name_fr">
            </div>
            <div class="flex w-100 justify-between p-5 gap-2">
                <button class="bg-green-600 border-black border-2 rounded text-white p-3 w-32"
                    @click="confirmEdit(category_venues.name, category_venues.name_en, category_venues.name_fr)">Conferma</button>
                <button class="bg-red-600 border-black border-2 rounded text-white p-3 w-32"
                    @click="showEditModal = false">Annulla</button>
            </div>
        </div>
    </ModalAction>

    <ModalAction :showModal="showModalDrink">
        <ShowDish :selectedDrink="selectedDrink" :allergens="allergens" :allergensDrinks="allergensDrinks"
            :category_venues="category_venues" :drink_category="drink_category" :pairingsEnoteca="pairingsEnoteca"
            :pairings="pairings" :drinks="drinks" @showModalDrink="showModalDrink = false"
            @deleteDrink="confirmDeleteDrink" @matchDish="matchAllergens" />
    </ModalAction>
...
export default {
    ...
    methods: {
        ...
        confirmAddDrinks() {
            ...
            addFieldToFormData('url', this.drink_category.url);
...
.grid-show-drink {
    ...
    grid-template-areas:
        ...
        "prezzo gradi immagine immagine"

...
<!-- MODALS -->
  ...
  <ModalAction :showModal="showEditModal">
      <div class="modal">
        <h2 class="h-20 font-bold text-2xl text-center">
          Modifica categoria
          {{ category_venues.find(category => category.id === categoryToEdit).name }}
          </h2>
...
export default {
  ...
  methods: {
        ...
        editCategory(id) {
            const categoryToEdit = this.localcategory_venues.find(category => category.id === id);
        
            const index = this.category_venues.findIndex(category => category.id === id);
            if (index !== -1) {
                this.category_venues[index] = { ...categoryToEdit };
            }
        
            // Mostra il modal di modifica
            this.showEditModal = true;
            this.categoryToEdit = id;
            this.categoryNameToEdit = categoryToEdit.name;
        },
